...
def evaluate_model(trainX, trainY, testX, testY):
    ...
    # n_timesteps, n_features, n_outputs = testX.shape[1], trainX.shape[2], trainY.shape[1]
...
if __name__ == '__main__':
    ...
    train_loss, train_acc = model.evaluate(trainX.reshape(-1, 4, 32, 6), trainY, batch_size = 32, verbose = 0)
    print('test_loss, test_acc:', train_loss, train_acc)
    # valid_loss, valid_acc = model.evaluate(_validx, _validy, batch_size = 32, verbose = 0)
    # print('valid_loss, valid_acc:', valid_loss, valid_acc)
    test_loss, test_acc = model.evaluate(testX.reshape(-1, 4, 32, 6), testY, batch_size = 32, verbose = 0)
    print('test_loss, test_acc:', test_loss, test_acc)

    train_out = model.predict(_trainx.reshape(-1, 4, 32, 6))
    test_out = model.predict(testX.reshape(-1, 4, 32, 6))
