...
if __name__ == '__main__':
    ...
    testFile, testX, testY = load_data('test')
    ...
    model = torch.load('./torchCNN')
    model.eval()
    pred = model(torch.tensor(testX))
    # print('pred', pred)
    # print('testY', testY)
    l_ = loss(pred, torch.tensor(testY))
    accuracy_ = accuracy(pred, torch.tensor(testY))
    print('loss={0}, accuracy={1}'.format(l_, accuracy_))
